...
print(f'Prophet分析sale数据\n{sales.head()}')
#print(sales.dtypes)#Date必须是datetime64格式
sales = sales.rename(columns = {'Date':'ds',
                                'Sales':'y'})#Prophet要求变量名字要是特定名称
#print(sales.head())

#2、262店铺日销售数据可视化
c = 'blue'
ax = sales.set_index('ds').plot(figsize=(30,4),color=c)#将日期作为横轴
ax.set_xlabel('日期')
ax.set_ylabel('日销售额')
plt.show()

#2、节日建模：Prophet允许专门对节日建模，分析节日的影响
...
holidays = holidays.drop_duplicates(subset='ds')#去重后(485, 2)
